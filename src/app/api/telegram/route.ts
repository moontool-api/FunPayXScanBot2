...
async function updateUserInDb(chatId: number, from: any, config: any): Promise<boolean> {
    ...
    try {
        ...
        const collection = mongoClient.db(dbName).collection(BOT_USERS_COLLECTION);
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (chatInfo && MONGODB_URI) {
        ...
        const user = await mongoClient.db(dbName).collection(BOT_USERS_COLLECTION).findOne({ chatId: chatInfo.id, ownerId: WORKER_ID });
    ...
    if (error.message.includes("bot was blocked by the user") && chatInfo?.id && MONGODB_URI) {
        let blockedUserMongoClient: MongoClient | undefined;
        try {
            blockedUserMongoClient = new MongoClient(MONGODB_URI);
            await blockedUserMongoClient.connect();
            const dbName = new URL(MONGODB_URI).pathname.substring(1) || MONGODB_DB_NAME;
            await blockedUserMongoClient.db(dbName).collection(BOT_USERS_COLLECTION).updateOne(
            ...
            if (blockedUserMongoClient) await blockedUserMongoClient.close();
        }
    }
