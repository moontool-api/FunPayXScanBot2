...
async function runExportProcess() {
    let localRedis: any;
    let localMongo: MongoClient | undefined;
    ...
    try {
        ...
        localMongo = new MongoClient(MONGODB_URI);
        await localMongo.connect();

        const dbName = new URL(MONGODB_URI).pathname.substring(1) || MONGODB_DB_NAME;
        const collection = localMongo.db(dbName).collection(COLLECTION_NAME);
        ...
        if (localMongo) await localMongo.close();
    }
}
